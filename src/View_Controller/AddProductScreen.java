...
/** Controller class for AddProductScreen.fxml.
 * @author Elexis Rox
 */
public class AddProductScreen implements Initializable {
    ...
    //Objects from AddProductScreen.fxml
    @FXML
    ...
    //Tables and Columns from AddProductScreen.fxml
    @FXML
    ...
    /** Initialize method for AddProductScreen that sets table columns and populates the tables with data.
     * @param url URL
    ...
    /** Saves the new Product to Inventory and parses all data to the correct fields. Catches exceptions and sets appropriate error messages to display.
    ...
    public void saveHandler(ActionEvent actionEvent) {
        ...
        for (int i = 0; i < Inventory.getAllProducts().size(); i++) {
            if (id <= Inventory.getAllProducts().get(i).getId())
                id = Inventory.getAllProducts().get(i).getId() + 2;
        }
        try { //Parsing fields
            ...
            for (int i=0; i<associatedTableList.size(); i++){
                addedProduct.addAssociatedPart(associatedTableList.get(i));
            }
